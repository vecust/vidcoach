...
- (UIImageView*) messageTextViewMaskView
{
	...
	{
		UIImage* shadowImage = [[UIImage imageNamed:@"TSAlertViewMessageListViewShadow2.png"] stretchableImageWithLeftCapWidth:6 topCapHeight:7];
...
- (CGSize) recalcSizeAndLayout: (BOOL) layout
{
	...
	if ( layout )
	{
		...
		if ( stacked )
		{
			...
			{
				b.frame = CGRectMake( kTSAlertView_LeftMargin, y, buttonWidth, buttonHeight * 2 );
				[self addSubview: b];
				y += (buttonHeight * 2) + kTSAlertView_RowMargin;
			}
		}
		else
		{
	...
	if (stacked) {
        return CGSizeMake( self.width, totalHeight * 1.55);
    } else {
